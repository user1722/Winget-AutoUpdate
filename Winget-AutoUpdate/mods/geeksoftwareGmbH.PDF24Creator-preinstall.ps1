...
$APPuninstall = "false"	
$APP32location = Join-Path -Path ${env:ProgramFiles} -ChildPath PDF24\pdf24.exe
$APP32location2 = Join-Path -Path ${env:Programdata} -ChildPath PDF24\pdf24.exe
$APP32location3 = Join-Path -Path ${env:ProgramFiles(x86)} -ChildPath PDF24\pdf24.exe
if ((Test-Path -Path $APP32location) -or (Test-Path -Path $APP32location2) -or (Test-Path -Path $APP32location3))
{
    $APPuninstall = "True"
...
if (Test-Path -Path $APP32location3)
{
$APP32locationuninstall = Join-Path -Path ${env:ProgramFiles(x86)} -ChildPath PDF24\unins001.exe
 Start-Process $APP32locationuninstall -ArgumentList "/S" -Wait
}
