...
function Invoke-PostUpdateActions
{
   ...
   if (!(Test-Path -Path $regPath -ErrorAction SilentlyContinue))
   {
      ...
      $null = (New-ItemProperty -Path $regPath -Name Publisher -Value 'Sturcz' -Force -Confirm:$False -ErrorAction SilentlyContinue)
      $null = (New-ItemProperty -Path $regPath -Name URLInfoAbout -Value 'https://github.com/user1722/Winget-AutoUpdate' -Force -Confirm:$False -ErrorAction SilentlyContinue)
